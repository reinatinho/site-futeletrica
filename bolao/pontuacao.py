from .models import Participante, Palpite, PalpiteClassificacao, PalpiteExtra, Jogo, Grupo, Selecao


def calcular_pontos_jogo(palpite_casa, palpite_fora, resultado_casa, resultado_fora):
    """
...
def calcular_pontos_classificacao(participante, grupo, classificacao_real):
    """
    ...
    real_dict = {idx + 1: item['id'] for idx, item in enumerate(classificacao_real)}

    for pos in range(1, 5):
        if palpite_dict.get(pos) == real_dict.get(pos):
...
def calcular_pontuacao_participante(participante):
    ...
    grupos = Grupo.objects.prefetch_related('selecoes', 'jogos').all()
    for grupo in grupos:
        jogos_grupo = grupo.jogos.filter(fase='grupos')
        jogos_com_resultado = jogos_grupo.filter(gols_casa__isnull=False, gols_fora__isnull=False)
        if jogos_com_resultado.count() == 6:
            classificacao_real = calcular_classificacao_real_grupo(grupo)
            pontos_classificacao += calcular_pontos_classificacao(
                participante, grupo, classificacao_real
            )
